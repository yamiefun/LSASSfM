...
double SfMAligner::ComputeEdgeWeight(const uint i, const uint j)
{
    ...
    LOG(INFO) << "Common images id: \n";
    for (const auto common_id : common_reg_images) {
        LOG(INFO) << common_id << std::endl;
    }
                             ...
                             double& msd)
{
    ...
    if (observations1.size() <= 5 || inliers1.size() <= 5) {
        ...
        for (size_t i = 0; i < observations1.size(); i++) {
                           ...
                           const Eigen::Matrix3d& R)
{
    ...
    const size_t n = src_rotations.size();
    ...
    double& scale)
{
    ...
    const size_t n = camera_centers1.size();
    ...
    for (size_t i = 0; i < camera_centers1.size() ;i++) {
